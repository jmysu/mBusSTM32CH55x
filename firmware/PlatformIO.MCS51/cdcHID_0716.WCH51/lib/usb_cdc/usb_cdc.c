...
void CDC_write(char c) {
  ...
  //EP2_bufCDC[64 + CDC_writePointer++] = c;              // write character
  EP2_bufCDCTX[CDC_writePointer++] = c;                 // write character
...
char CDC_read(void) {
  ...
  data = EP2_bufCDC[CDC_readPointer++];                 // get character
...
// Setup CDC endpoints
void CDC_setup(void) {
  UEP2_DMA    = (uint16_t)EP2_bufCDC;       // EP2 data transfer address
  //UEP3_DMA    = (uint16_t)EP3_bufCDC;       // EP3 data transfer address
    ...
    switch(USB_setupBuf->bRequest) {
      case GET_LINE_CODING:                   // 0x21  currently configured
        for(i=0; i<sizeof(CDC_lineCodingB); i++)
          EP0_bufUSB[i] = ((uint8_t*)&CDC_lineCodingB)[i]; // transmit line coding to host
        return sizeof(CDC_lineCodingB);
      case SET_CONTROL_LINE_STATE:            // 0x22  generates RS-232/V.24 style control signals
        CDC_controlLineState = EP0_bufUSB[2]; // read control line state
  ...
  if(SetupReq == SET_LINE_CODING) {                         // set line coding
    if(U_TOG_OK) {
      for(i=0; i<((sizeof(CDC_lineCodingB)<=USB_RX_LEN)?sizeof(CDC_lineCodingB):USB_RX_LEN); i++)
        ((uint8_t*)&CDC_lineCodingB)[i] = EP0_bufUSB[i];    // receive line coding from host
