...
void USB_EP0_copyDescr(uint8_t len) {
  ...
  __asm
    ...
    mov  dptr, #_EP0_bufUSB     ; dptr1 <- EP0_bufUSB
    ...
    inc  dptr                   ; inc dptr0
    .DB  0xA5                   ; acc -> EP0_bufUSB[dptr1] & inc dptr1
...
void USB_init(void) {
  ...
  UEP0_DMA    = (uint16_t)EP0_bufUSB;       // EP0 data transfer address
