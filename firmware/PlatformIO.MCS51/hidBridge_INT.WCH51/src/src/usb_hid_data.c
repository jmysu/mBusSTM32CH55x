...
void HID_write(uint8_t c) {
  ...
  //EP1_bufHID[64 + HID_writePointer++] = c;      // write byte to buffer
  EP1_bufHIDTX[HID_writePointer++] = c;      // write byte to buffer
...
uint8_t HID_read(void) {
  ...
  data = EP1_bufHID[HID_readPointer++];         // get character
...
void HID_EP_init(void) {
  UEP1_DMA    = (uint16_t)EP1_bufHID;           // EP1 data transfer address
