...
void CDC_write(char c) {
  ...
  //EP2_bufCDCTX[64 + CDC_writePointer++] = c;      // write character to buffer
  EP2_bufCDCTX[CDC_writePointer++] = c;      // write character to buffer
...
char CDC_read(void) {
  ...
  //data = EP1_buffer[CDC_readPointer++];           // get character
  data = EP2_bufCDCRX[CDC_readPointer++];           // get character
...
void CDC_EP2_init(void) {
 //UEP1_DMA    = (uint16_t)EP1_buffer;             // EP1 data transfer address

  UEP2_DMA    = (uint16_t)EP2_bufCDCRX;             // EP2 data transfer address
  ...
  switch(USB_SetupReq) {
    case GET_LINE_CODING:                         // 0x21  currently configured
      for(i=0; i<sizeof(CDC_lineCoding); i++)
        EP0_bufCNTRL[i] = ((uint8_t*)&CDC_lineCoding)[i]; // transmit line coding to host
      return sizeof(CDC_lineCoding);
    case SET_CONTROL_LINE_STATE:                  // 0x22  generates RS-232/V.24 style control signals
      CDC_controlLineState = EP0_bufCNTRL[2];       // read control line state
      return 0;
...
void CDC_EP0_OUT(void) {
  ...
  if(USB_SetupReq == SET_LINE_CODING) {           // set line coding
    for(i=0; i<((sizeof(CDC_lineCoding)<=USB_RX_LEN)?sizeof(CDC_lineCoding):USB_RX_LEN); i++)
      ((uint8_t*)&CDC_lineCoding)[i] = EP0_bufCNTRL[i];      // receive line coding from host
  }
