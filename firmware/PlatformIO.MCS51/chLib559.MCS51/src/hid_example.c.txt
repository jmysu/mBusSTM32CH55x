...
static uint8_t get_descriptor_size(uint8_t type, uint8_t no) __reentrant{
  switch (type) {
...
static const uint8_t* get_descriptor(uint8_t type, uint8_t no) __reentrant{
  switch (type) {
    case USB_DESC_DEVICE:
      return device_desc;
    case USB_DESC_CONFIGURATION:
...
static void get_report(uint8_t* buffer, uint8_t* len) __reentrant{
  buffer[0] = 0;
  buffer[1] = 0;
  buffer[2] = 0;
