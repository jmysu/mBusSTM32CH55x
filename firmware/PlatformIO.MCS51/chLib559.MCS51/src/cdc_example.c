...
static uint8_t get_string_length(uint8_t no) __reentrant{
  switch (no) {
    case 1:
...
static const char* get_string(uint8_t no) __reentrant{
  switch (no) {
    case 1:
...
void recv(const uint8_t* buffer, uint8_t len) __reentrant{
  // Echo back
