...
void main(void) {
  ...
  // Loop
  while(1) {
    if (VEN_BOOT_flag)  {BOOT_prepare(); DLY_ms(100); BOOT_now();}            // enter bootloader?
    if (VEN_LED16_flag) PIN_toggle(P16);  else PIN_low(P16);
    if (VEN_LED17_flag) PIN_toggle(P17);  else PIN_low(P17);
