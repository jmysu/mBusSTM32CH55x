...
void USB_EP_init(void) {
  UEP0_DMA    = (uint16_t)EP0_bufCNTRL;       // EP0 data transfer address
...
// Fast Copy Function
// ===================================================================================
// Copy descriptor *USB_pDescr to EP0_bufCNTRL using double pointer
...
void USB_EP0_copyDescr(uint8_t len) {
  ...
  __asm
    ...
    mov  dptr, #_EP0_bufCNTRL     ; dptr1 <- EP0_bufCNTRL
    ...
    inc  dptr                   ; inc dptr0
    .db  0xA5                   ; acc -> EP0_bufCNTRL[dptr1] & inc dptr1
